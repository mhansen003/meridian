...
interface DossierResult {
  dossier: { competitor: string; signalIds: string[]; summary: string; lastUpdated: string };
  summary: string;
  strengths: string[];
  threats: string[];
  ...
  return (
    <div className="max-w-4xl mx-auto px-6 py-8">
      ...
      <p className="text-white/40 text-sm mb-8">
        Signals tagged with competitor mentions, synthesized into competitive dossiers.
      </p>
      ...
      {competitors.length === 0 ? (
        <div className="p-8 rounded-xl border border-white/8 text-center">
          ...
          <p className="text-xs text-white/25 mt-1">Tag signals as competitor intel in the Pulse feed.</p>
        </div>
      ) : (
          ...
          {competitors.map((group) => {
            ...
            return (
              <div key={group.competitor} className="rounded-2xl border border-white/10 bg-white/3 overflow-hidden">
                ...
                <div className="p-5 flex items-center justify-between gap-3">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                      ...
                      <p className="text-xs text-white/40">{group.count} signals</p>
                    </div>
                  </div>
                ...
                {isExpanded && (
                  <div className="px-5 pb-5 border-t border-white/8 pt-4">
                    <p className="text-xs text-white/30 uppercase tracking-wider mb-3">Source Signals</p>
                    <div className="space-y-2">
                      {group.signals.map((s) => <SignalCard key={s.id} signal={s} />)}
                    </div>
                  </div>
                )}
              </div>
